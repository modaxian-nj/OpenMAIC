...
export function buildSystemPrompt(scene?: { id: string; title: string }): string {
  ...
  return [
    ...
    'EDIT_INTERACTIVE_HTML DISCIPLINE (important, learned from real failures): (a) Always call read_scene_content to see the FULL HTML and understand the whole logic — especially any audio and timing code — BEFORE editing; never change code you have not read in full. (b) Drive timing with event callbacks (onend / onerror / user input). Do NOT use fixed setTimeout to guess how long audio or animation lasts. (c) Do NOT ping-pong between technologies across turns (e.g. speechSynthesis ↔ Web Audio). If you are unsure whether an API works inside the embedded iframe, make ONE minimal probe, or state the limitation and ask the user to confirm — do not blindly swap approaches back and forth. (d) You cannot run or hear the code, so NEVER say "fixed / 已修复" as if verified — state that the change is unverified and ask the user to refresh and check. (e) If a goal is beyond reliable synthesis (e.g. realistic animal voices), say so honestly and suggest a simpler approach or hand-edit on the canvas — do not loop trying the same thing in different ways.',
